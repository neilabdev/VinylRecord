...
Above, the **belongs_to_dec** and **belongs_to_imp** take three or four parameters: the model name ('Group'),   getter name ('group'), optionally the required column name which is assumed to be the getter name plus 'Id' suffix, or you may specify if different as the model Class name lowercase first character, plus 'Id' suffix, followed by a destruction method of ARDependencyDestroy to delete the object, or ARDependencyNullify to null the object. The main thing to remember when describing the column key of relationship, it must match the name of the model and begin with lowercased letters. Examples follow:
